...
from src.legacy.TeamB1pomt5.code.omsignal.utils.pytorch_utils import (
    log_training,
    get_id_mapping,
    map_ids,
)
from src.legacy.TeamB1pomt5.code.omsignal.utils.dataloader_utils import (
    import_train_valid,
    import_OM,
    OM_dataset,
    get_dataloader,
)
from src.legacy.TeamB1pomt5.code.omsignal.utils.augmentation import (
    RandomCircShift,
    RandomDropoutBurst,
    RandomNegate,
    RandomReplaceNoise,
...
def load_data(model_hp_dict, gpu_avail, device):
    ...
    # Add transformations
    trsfrm = transforms.RandomChoice(
        [
            RandomCircShift(0.5),
            RandomNegate(0.5),
            RandomReplaceNoise(0.5),
            RandomDropoutBurst(0.5),
        ]
    )

    train_loader, valid_loader = fetch_dataloaders(
        X_train, y_train, X_valid, y_valid, model_hp_dict["batchsize"], transform=trsfrm
    )
...
def fetch_dataloaders(
    train_data, train_labels, valid_data, valid_labels, batch_size=50, transform=None
):  # No need with train_ID_CNN
    """
    ...
    train_loader = get_dataloader(
        train_data, train_labels, transform, batch_size=batch_size
    )
    valid_loader = get_dataloader(
        valid_data, valid_labels, transform, batch_size=batch_size
    )  # Only uses original validation data
...
def get_dataloader_unlabeled(
    X, transform, batch_size, shuffle=True, task_type="Regression"
):

    if task_type not in ["Regression", "Classification", "Ranking"]:
        raise ValueError(
            "task_type must be in ['Regression', 'Classification' , 'Ranking']"
        )
