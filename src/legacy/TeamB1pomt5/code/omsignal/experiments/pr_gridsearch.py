...
from omsignal.utils.pytorch_utils import get_id_mapping, get_dataloader, map_ids, train_PR_CNN
...
if __name__ == '__main__':
    ...
    for params in grid:
        try:
            PR_CNN, _, pr_valid_losses = train_PR_CNN(X_train, y_train, X_valid, y_valid, 
        ...
        min_val_loss = min(pr_valid_losses)
    ...
    torch.save(best_model.state_dict(), os.path.join(MODELS_DIR, 'best_PR_CNN.pt'))

    # Log this info - just in case retraining is needed by hand
    with open(os.path.join(LOG_DIR, 'PR_best_hyperparams.txt'), 'w') as fp:
