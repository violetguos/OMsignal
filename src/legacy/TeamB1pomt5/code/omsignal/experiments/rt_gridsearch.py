...
from omsignal.utils.pytorch_utils import get_id_mapping, get_dataloader, map_ids, train_RT_Ranker
...
if __name__ == '__main__':
    ...
    for params in grid:
        try:
            RT_Ranker, _, rt_valid_losses, _, _ = train_RT_Ranker(X_train, y_train, X_valid, y_valid, 
        ...
        min_val_loss = min(rt_valid_losses)
        if min_val_loss < current_min:
            current_min = min_val_loss
            best_config = params
            best_model = RT_Ranker
    ...
    torch.save(best_model.state_dict(), os.path.join(MODELS_DIR, 'best_RT_Ranker.pt'))

    # Log this info - just in case retraining is needed by hand
    with open(os.path.join(LOG_DIR, 'RT_best_hyperparams.txt'), 'w') as fp:
