...
_TSITOURAS_TABLEAU = _ButcherTableau(
    alpha=[0.161, 0.327, 0.9, 0.9800255409045097, 1.0, 1.0],
    beta=[
        [0.161],
...
def _interp_eval_tsit5(t0, t1, k, eval_t):
    ...
    y_t = tuple(
        y0_ + _scaled_dot_product(dt, interp_coeff, k_) for y0_, k_ in zip(y0, k)
    )
    return y_t
...
class Tsit5Solver(AdaptiveStepsizeODESolver):
    def __init__(
        ...
        **unused_kwargs
        ...
        self.max_num_steps = _convert_to_tensor(
            max_num_steps, dtype=torch.int32, device=y0[0].device
        )

    def before_integrate(self, t):
        if self.first_step is None:
            first_step = _select_initial_step(
                self.func, t[0], self.y0, 4, self.rtol, self.atol
            ).to(t)
        else:
            first_step = _convert_to_tensor(0.01, dtype=t.dtype, device=t.device)
        self.rk_state = _RungeKuttaState(
            self.y0,
            self.func(t[0].type_as(self.y0[0]), self.y0),
            t[0],
            t[0],
            first_step,
    ...
    def advance(self, next_t):
        ...
        while next_t > self.rk_state.t1:
            ...
            n_steps += 1
        return _interp_eval_tsit5(
            self.rk_state.t0, self.rk_state.t1, self.rk_state.interp_coeff, next_t
        )
    ...
    def _adaptive_tsit5_step(self, rk_state):
        ...
        y1, f1, y1_error, k = _runge_kutta_step(
            self.func, y0, f0, t0, dt, tableau=_TSITOURAS_TABLEAU
        )
        ...
        ########################################################
        error_tol = tuple(
            self.atol + self.rtol * torch.max(torch.abs(y0_), torch.abs(y1_))
            for y0_, y1_ in zip(y0, y1)
        )
        tensor_error_ratio = tuple(
            y1_error_ / error_tol_ for y1_error_, error_tol_ in zip(y1_error, error_tol)
        )
        sq_error_ratio = tuple(
            torch.mul(tensor_error_ratio_, tensor_error_ratio_)
            for tensor_error_ratio_ in tensor_error_ratio
        )
        mean_error_ratio = sum(
            torch.sum(sq_error_ratio_) for sq_error_ratio_ in sq_error_ratio
        ...
        dt_next = _optimal_step_size(
            dt, mean_error_ratio, self.safety, self.ifactor, self.dfactor
        )
