...
class Decoder(torch.nn.Module):
    def __init__(self, d_in, d_out, use_cuda):
        ...
        if self.d_out is not None:
            # TODO: try using Transpose conv2d and MaxUnpool
    ...
    def g(self, tilde_z_l, u_l):
        ...
        v_l = torch.mul(b_a6, torch.sigmoid(torch.mul(b_a7, u_l) + b_a8)) + \
              ...
              b_a10
        # print("***********")
        # print("tilde_z_l", tilde_z_l.shape)
        # print("u_l", u_l.shape)
        # print("v_l", v_l.shape)

        # TODO:
        # tilde_z_l torch.Size([32, 1024]) 1024 is the decoder size defined by me
        # not sure how it should be broadcasted
        # the key here is to understand ladder network and see what is the real dimension
...
class StackedDecoders(torch.nn.Module):
    ...
    def bn_hat_z_layers(self, hat_z_layers, z_pre_layers):
        ...
        for i, (hat_z, z_pre) in enumerate(zip(hat_z_layers, z_pre_layers)):
            ...
            # print("onex", ones.shape)
            # print(" ones.mm(mean)",  ones.mm(mean).shape)
            # print("ones.mm(torch.sqrt(var + 1e-10))", ones.mm(torch.sqrt(var + 1e-10)).shape)
